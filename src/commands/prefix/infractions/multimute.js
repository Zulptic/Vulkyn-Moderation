...
export default {
    ...
    async execute(message, args, client) {
        ...
        for (const id of ids) {
            ...
            if (muteError) {
                await errorService.commandError(client, muteError, message, 'multimute:add-role', { targetId: id, muteRoleId });
            ...
            if (!infraction) {
                await member.roles.remove(muteRole, 'Mute logging failed; rolling back').catch(err =>
                    errorService.commandError(client, err, message, 'multimute:rollback-role', { targetId: id, muteRoleId })
                );
